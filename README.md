...
| 审核结果示例 1 | 审核结果示例 2 |
|:---:|:---:|
| ![审核结果示例1](docs/screenshots/review-panel.png) | ![审核结果示例2](docs/screenshots/review-result.png) |

---

## 功能特色

🔍 **Word 原生体验** — 审核结果直接以 Word 批注呈现，不离开编辑环境，阅读修改一气呵成

🎨 **四级风险标注** — 严重(红)、高(橙)、中(黄)、低(蓝)，批注颜色一目了然，优先处理高风险条款

🤖 **灵活接入大模型** — 兼容任意 OpenAI 格式 API，自由切换模型、调整提示词，适配不同业务场景

📋 **全文 / 选中审核** — 支持一键审核整篇合同，也可选中关键段落精准审核；选中审核精度更高，适合信息密度大的条款

📊 **一键导出报告** — 审核结果可导出为结构化文本报告，方便归档和团队协作

⚡ **断点续传与重试** — 网络波动自动重试，大文档审核中途取消后可从断点继续

🧪 **零成本体验** — 内置测试模式，无需 API Key 即可用 Mock 数据完整体验全部功能

🌗 **深色模式适配** — 自动跟随 Office / 系统主题切换明暗配色
...
- 点击 **审核全文** — 审核整篇合同
- 点击 **审核选中内容** — 仅审核你在 Word 中选中的内容
- 开启 **测试模式** — 使用 Mock 数据，不调用真实 API，适合先体验功能

> 💡 **提示**："审核选中内容"会增加审核的精度，适用于信息密度较大的条款。建议对关键条款单独选中审核以获得更细致的分析结果。
